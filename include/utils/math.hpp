 ...
 * @return
 */
std::array<GLfloat, 6> findBounds(const std::vector<glm::vec3> &points);
 ...
 * @return
 */
Eigen::Matrix3f build_covarience_matrix(const std::vector<glm::vec3> &points);
 ...
 * @return
 */
std::vector<glm::vec2> rotate_points(const std::vector<glm::vec2> &points, size_t idx);
                                 ...
                                 const glm::vec3 &rot_axis, const glm::vec3 &sizes)
{
    ...
    const glm::vec3 scale = sizes;

    glm::mat4 ident = glm::mat4(1.f);
    glm::mat4 scaling = glm::scale(ident, scale);
    glm::mat4 translation = translate(ident, pos);
    glm::mat4 rotation =
        rotate_around(ident, glm::vec3(centerX, centerY, centerZ), angle, rot_axis);
                                                ...
                                                const glm::mat4 &transform)
{
    ...
    for (const glm::vec3 &v : vertices)
        res.emplace_back(transform * glm::vec4(v, 1.f));
                                                ...
                                                const glm::mat4 &transform)
{
    ...
    for (const auto &v : triangles) {
        glm::vec3 a = glm::vec3(v.p0[0], v.p0[1], v.p0[2]);
        glm::vec3 b = glm::vec3(v.p1()[0], v.p1()[1], v.p1()[2]);
        glm::vec3 c = glm::vec3(v.p2()[0], v.p2()[1], v.p2()[2]);

        a = transform * glm::vec4(a, 1.f);
        b = transform * glm::vec4(b, 1.f);
        c = transform * glm::vec4(c, 1.f);
...
inline glm::vec3 computeCentroid(const std::vector<glm::vec3> &points)
{
    glm::vec3 means{0.f};
...
constexpr glm::vec3 viewportToNDC(const glm::vec2 &pos, const glm::vec2 &clip)
{
...
constexpr GLfloat barry_centric(const glm::vec3 &p1, const glm::vec3 &p2, const glm::vec3 &p3,
                                const glm::vec2 &pos)
{
