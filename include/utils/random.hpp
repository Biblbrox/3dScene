...
class Random {
  public:
    ...
    void fill_unique(ForwardIterator begin, ForwardIterator end, T left, T right,
                     bool fix_near = false)
    {
        std::generate(begin, end, [begin, end, left, right, fix_near, this]() {
            T val = generateu<T>(left, right);
            if (fix_near) {
                while (std::count(begin, end, val) != 0 || std::count(begin, end, val - 1) != 0 ||
    ...
    void fill_gauss(ForwardIterator begin, ForwardIterator end, T mean, T deviation)
    {
        std::generate(begin, end,
                      [mean, deviation, this]() { return generaten<T>(mean, deviation); });
    }
    ...
    {
        std::generate(begin, end, [left, right, this]() { return generateu<T>(left, right); });
    }
     ...
     */
    template <typename T>
    typename std::enable_if<std::is_arithmetic_v<T>, T>::type generateu(T a, T b)
    {
     ...
     */
    template <typename T>
    typename std::enable_if<std::is_floating_point_v<T>, T>::type generaten(T mean, T std)
    {
