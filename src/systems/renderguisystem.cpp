...
RenderGuiSystem::RenderGuiSystem()
    : m_videoSettingsOpen(false), m_colorSettingsOpen(false), m_exportSettingsOpen(false),
      m_laserSettingsOpen(false), m_openExportDialog(false), m_selSettingsOpen(false)

{
    ...
    if (!io.Fonts->IsBuilt()) {
        ...
        m_font = io.Fonts->AddFontFromFileTTF(
            getResourcePath("fonts/NotoSans-hinted/NotoSans-Medium.ttf").c_str(), 15.f, NULL,
...
void RenderGuiSystem::update_state(size_t delta)
{
    ...
    if (sceneComp->isMsaa) {
        ...
        glBlitFramebuffer(0, 0, size.x, size.y, 0, 0, size.x, size.y, GL_COLOR_BUFFER_BIT,
                          GL_NEAREST);
    }
    ...
    bool open = true;
    ImGui::Begin("GameWindow", &open,
                 ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar |
                     ImGuiSliderFlags_AlwaysClamp | ImGuiWindowFlags_NoScrollWithMouse |
                     ...
                     ImGuiWindowFlags_MenuBar);
    {
                   ...
                   {0, -1});
        {
            ...
            if (Button(_("Calibrate camera")))
                Config::getVal<bool>("CalibrateCameraPressed") = true;
            ...
            if (m_colorSettingsOpen) {
                ...
                ColorPicker4("##Background color",
                             glm::value_ptr(Config::getVal<vec4>("BackgroundColor")));
                End();
            }
            ...
            if (viewport_size != size)
                getEntitiesByTag<SceneComponent>()
                    .begin()
                    ->second->getComponent<SceneComponent>()
                    ->dirty = true;
...
void RenderGuiSystem::video_settings()
{
    ...
    if (m_videoSettingsOpen) {
        ...
        if (Checkbox(_("Enable antialiasing(Need game restart)"), &Config::getVal<bool>("MSAA")) ||
            msaaEnabled) {
...
void RenderGuiSystem::add_model()
{
    ...
    if (Button(_("Add model"))) {
        ...
        ImGuiFileDialog::Instance()->OpenDialog("ChooseModelDlgKey", _("Choose model file"),
                                                filters, ".");
    }
...
void RenderGuiSystem::selection_settings()
{
    ...
    std::string new_ent_name = selEn->name();
    if (InputText("##entity_name", &new_ent_name)) {
        if (new_ent_name != selEn->name())
            setEntityName(selEn->name(), new_ent_name);
