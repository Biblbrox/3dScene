...
{
protected:
    static std::shared_ptr<ArcballCamera> instance;
public:
    ArcballCamera(const glm::vec3& pos = glm::vec3(10.f, 10.f, 10.f),
           const glm::vec3& target = glm::vec3(0.f, 0.f, 0.f),
           const glm::vec3& up = glm::vec3(0.f, 1.f, 0.f));

    static std::shared_ptr<ArcballCamera> getInstance()
    {
        if (!instance)
            instance = std::make_shared<ArcballCamera>();
